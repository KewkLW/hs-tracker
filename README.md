...
Season 10 farming and run analysis. The changes are grouped here so it
...
| **Documentation and tests** — this overview, detailed compatibility notes, JavaScript XP tests, Rust run/level regression tests, and corrected package-lock identity/version. | The behavior, estimates, and remaining limitations need to be reviewable and reproducible instead of living only in commit history. The current branch passes 4 JavaScript and 124 Rust tests, the production Vite build, and strict Rust Clippy. |
